...
def upload_file(file_path, origin=None):
    ...
    # first try to add origin to already existing file
    response = requests.post(url, data=data, verify=False)
    if response.status_code == 200:
        pass
    elif response.status_code == 400 and 'Invalid hash length' in response.text:
        print(f"Hash {sha256} of {file_path} is an invalid hash?")
        ...
        if response.status_code == 409 and 'already exists' in response.text:
            ...
            # change backend DB to not get (sqlite3.OperationalError) database is locked - should not happen anymore
            time.sleep(1)
            return upload_file(file_path, origin)
        elif response.status_code != 200:
            print(f"Failed to upload file '{file_path}'. Status code: {response.status_code} and response: {response.text}")
    else:
        print(f"Failed to get file id for {file_path} hash {sha256}. Status code: {response.status_code} and response: {response.text}")
